...
def exibir_imagem(caminho, largura=None):
    ...
    try:
        ...
        st.error(f"Ocorreu um erro ao exibir a imagem: {str(e)}")
...
caminho =  "https://raw.githubusercontent.com/PedroArroyoCruz/Abastecimento_Estoque/main/imagem/Logo-AsMara-nova-cor_low-1024x937-optimized.webp"
    ...
    if dados_corretos:
        with tab3:
            ...
            if 'bases_aba3' not in st.session_state:
                ...
                # Cria um novo buffer de bytes para o arquivo XLSX
                novo_buffer = io.BytesIO()

                # Salva o DataFrame no novo buffer
                with pd.ExcelWriter(novo_buffer, engine='xlsxwriter') as writer:
                    df_apos_abastecimento_sku.to_excel(writer, sheet_name='Sheet1', index=False)

                # Move o cursor para o início do novo buffer
                novo_buffer.seek(0)
                ...
                st.session_state.bases_aba3 = {
                        ...
                        'excel_buffer': buffer,
                        'excel_buffer_novo':novo_buffer,
                        'associacoes' : associacoes
                    }
                buffer = st.session_state.bases_aba3.get('excel_buffer')
                novo_buffer = st.session_state.bases_aba3.get('excel_buffer_novo')
